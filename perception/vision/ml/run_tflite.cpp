...
// Read each line of float from a text file into vector<float>
...
void fillData(std::uint8_t* inputImgsBuffer, std::uint8_t*bigInputImgsBuffer) {
    // Input: 1, 12, 128, 256 NCHW, Output: 1, 128, 256, 12 NHWC

    std::string inputImgFilename = "test/input/input_imgs-random.csv";
    std::string bigInputImgFilename = "test/input/big_input_imgs-random.csv";

    std::vector<float> inputImgInput = readFloatsFromFile(inputImgFilename);
    std::vector<float> bigInputImgInput = readFloatsFromFile(bigInputImgFilename);

    for (int c=0; c<12; ++c) {
        for (int h=0; h<128; ++h) {
            for (int w=0; w<256; ++w) {
                int inputOffset = c * (128 * 256) + h * 256 + w;
                int outputOffset = h * (256 * 12) + w * 12 + c;

                inputImgsBuffer[outputOffset] = static_cast<std::uint8_t>(inputImgInput[inputOffset]);
                bigInputImgsBuffer[outputOffset] = static_cast<std::uint8_t>(bigInputImgInput[inputOffset]);
            }
        }
    }
}
...
int main(int argc, char * argv[]) {
    ...
    fillData(inputImgsBuffer, bigInputImgsBuffer);
