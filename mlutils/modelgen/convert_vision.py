...
class Results:
    ...
    def generate_report(self):
        ...
        if _CFG["verbose"]:
            print(
                f"Writing {_CFG['report_filename']}{' (dry_run)' if _CFG['dry_run'] else ''}\n"
            )
        self._report(
            f"*Report generated at {datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%SZ')}{_CFG['github_actions_md']}.*"
        )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Test converting ONNX models to TensorFlow."
    )
    parser.add_argument(
        "-m",
        "--models",
    ...
    parser.add_argument(
        "-o", "--output", default=tempdir, help=f"output directory (default: {tempdir})"
    )
