...
def get_leads(model_output):
    leads = []
    for i in range(3):
        lead = Lead()
        fill_xyvat(lead, ModelConstants.LEAD_T_IDXS, *model_output['lead'][0,i].T, *model_output['lead_stds'][0,i].T)
        lead.prob = model_output['lead_prob'][0,i].tolist()
